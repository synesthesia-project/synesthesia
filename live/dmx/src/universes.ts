...
import { CONFIG_DELETED, ConfigNode } from '@synesthesia-project/live-core/lib/util';
...
export class Universes {
  ...
  public constructor(
    private readonly config: ConfigNode<UniversesConfig>,
  ) {
    ...
    this.group
      ...
      .addListener(() => {
        this.config.update((config) => config.slice(0, config.length - 1));
      });

    config.addListener('change', (config) => {
