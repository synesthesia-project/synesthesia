...
const HALO_INPUT_CONFIG = t.type({
  /** What proportion of the total travel time should be  */
  speed: t.number,
  thickness: t.number,
  
});

type Config = t.TypeOf<typeof HALO_INPUT_CONFIG>;

const createHaloInput = (context: InputContext<Config>): Input<Config> => {
  const state: {
    config: Config;
  } = {
    config: {
...
export const HALO_INPUT_KIND: InputKind<Config> = {
  kind: 'halo',
  config: HALO_INPUT_CONFIG,
  initialConfig: {
    advanceAmountPerSecond: 0.3,
    sequence: [],
  },
  create: createHaloInput,
};

export const HALO_INPUT_PLUGIN: Plugin = {
  init: (context) => {
    context.registerInputKind(HALO_INPUT_KIND);
  },
};
